    ...
    if ($volunteersResponse['http_code'] === 200) {
        ...
        if (! empty($volunteers)) {
            ...
            if (empty($firstVolunteer['birth_date'])) {
                $issues[] = 'Missing birth_date field (Issue 6b)';
            }
            if (empty($firstVolunteer['place_of_birth'])) {
                $issues[] = 'Missing place_of_birth field (Issue 6b)';
            }
            if (empty($firstVolunteer['phone'])) {
                $issues[] = 'Missing phone field (Issue 6b)';
            }
            if (empty($firstVolunteer['member_number'])) {
                $issues[] = 'Missing member_number field (Issue 6b)';
            }
    ...
    if (! empty($volunteers)) {
        ...
        if ($userResponse['http_code'] === 200) {
            ...
            if (empty($user['birth_date'])) {
                $missingFields[] = 'birth_date (tanggal_lahir_pengguna)';
            }
            if (empty($user['place_of_birth'])) {
                $missingFields[] = 'place_of_birth (tempat_lahir_pengguna)';
            }
            if (empty($user['phone'])) {
                $missingFields[] = 'phone (no_handphone_pengguna)';
            }
            if (empty($user['member_number'])) {
                $missingFields[] = 'member_number (no_anggota)';
            }

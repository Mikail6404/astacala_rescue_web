        ...
        <div class="flex-1 p-8 overflow-y-auto">
            ...
            @if(isset($report))
                <div class="bg-white shadow rounded p-6">
                    <p class="mb-3"><strong>Judul:</strong> {{ $report['title'] ?? '-' }}</p>
                    <p class="mb-3"><strong>Lokasi:</strong> {{ $report['location_name'] ?? '-' }}</p>
                    <p class="mb-3"><strong>Koordinat:</strong> {{ $report['coordinates'] ?? '-' }}</p>
                    <p class="mb-3"><strong>Jenis Bencana:</strong> {{ $report['disaster_type'] ?? '-' }}</p>
                    <p class="mb-3"><strong>Tingkat Keparahan:</strong> {{ $report['severity_level'] ?? '-' }}</p>
                    <p class="mb-3"><strong>Deskripsi:</strong> {{ $report['description'] ?? '-' }}</p>
                    <p class="mb-3"><strong>Dibuat Oleh:</strong> {{ $report['team_name'] ?? '-' }}</p>
                    <p class="mb-3"><strong>Status:</strong> {{ $report['status'] ?? '-' }}</p>
                    <p class="mb-3"><strong>Tanggal:</strong> {{ $report['created_at'] ?? '-' }}</p>
                ...
                <a href="{{ route('pelaporan.index') }}" class="mt-6 inline-block bg-gray-300 hover:bg-gray-400 text-gray-800 font-semibold py-2 px-4 rounded">Kembali ke Pelaporan</a>
            @else
